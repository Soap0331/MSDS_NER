...
class NERDataset(Dataset):
    def __init__(self, texts, tags, tokenizer, tag2id, max_len=128):
        ...
        # 确保使用的是FastTokenizer
        if not hasattr(tokenizer, 'backend_tokenizer') or not tokenizer.backend_tokenizer:
            raise ValueError("必须使用FastTokenizer版本，如BertTokenizerFast")
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # 映射标签的两种方法：
            # 1. 只标记每个单词的第一个子词（原始方法）
            if True:  # 可以切换为False以使用第二种方法
...
def predict_sentence(model, tokenizer, sentence, tag2id, id2tag, device):
    ...
    # The issue is here - word_ids() might return None in some cases
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default='bert_ner_model', help='Directory to save model and results')
    ...
    # 加载BERT分词器（使用Fast版本）
    try:
        print(f"加载 {args.bert_model} 的Fast分词器...")
        tokenizer = BertTokenizerFast.from_pretrained(args.bert_model)
    except Exception as e:
        print(f"加载Fast分词器失败: {e}")
        print("尝试安装需要的依赖...")
        import subprocess
        subprocess.call(["pip", "install", "tokenizers"])
        print("重新尝试加载Fast分词器...")
        tokenizer = BertTokenizerFast.from_pretrained(args.bert_model)
    ...
    test_sentences = [
        ...
        "ld50 lc50 mixture:oral ld50 rats = 840 mg\/kg. reports of carcinogenicity:ntp:no iarc:no osha:no",
...
if __name__ == "__main__":
    main()
    #python bert-ner-train.py --data_path output0520_enhance.bio --output_dir ner_model_new --epochs 10 --patience 5
    #python bert-ner-train.py --data_path output0518.bio --output_dir ner_model0518without_enhance --epochs 10 --patience 3